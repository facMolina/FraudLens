...
> ✅ **Regla del equipo (2026-09-16):** se trabaja **directo sobre `main`** — commit y push
> directo, **sin Pull Request**.
>
> Hasta el 16/9 se trabajó sobre `claude/proyecto-base-documentacion-8njz5t` (nombre de una sesión
> de Claude Code, no pensado para que todo el equipo lo vea como rama de trabajo). Se creó `main`
> desde ese mismo estado — mismo historial, nada se perdió — y se dejó como rama default del repo
> en GitHub. La rama vieja queda de respaldo.
>
> Además se decidió no abrir más Pull Requests: dos integrantes abrieron PRs desde ramas propias
> (`mdv/fix-...`, `encuestas`) y ninguno pudo cerrarlos solo — hubo que mergearlos a mano y resolver
> conflictos en `historial-aportes.md`. Para un equipo de 4 personas trabajando en Markdown, el PR
> agrega fricción sin agregar revisión real (nadie lo estaba revisando antes de pedir el merge).
>
> ⚠️ Si tu sesión de Claude Code (o la de otro integrante) te propone por defecto una rama nueva y
> un PR, avisale que trabaje directo sobre `main` — esa configuración depende de cómo se armó tu
> sesión/entorno, este archivo no la puede cambiar por vos.

| Situación | Qué hacer |
|---|---|
| Editar documentación, cargar una clase, sumar bitácora | Directo a `main` |
| Antes de empezar a trabajar | `git pull origin main` — evita pisar el trabajo de otro |
